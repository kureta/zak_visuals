...
from Qt import QtCore, QtWidgets  # noqa
...
def on_nodeEdited(nodeName, newName):
    print(f'node edited : {nodeName}, new name : {newName}')
...
def on_nodeMoved(nodeName, nodePos):
    print(f'node {nodeName} moved to {nodePos}')


@QtCore.Slot(str)
def on_nodeDoubleClick(nodeName):
    print(f'double click on node : {nodeName}')


# Attrs
@QtCore.Slot(str, int)
def on_attrCreated(nodeName, attrId):
    print(f'attr created : {nodeName} at index : {attrId}')


@QtCore.Slot(str, int)
def on_attrDeleted(nodeName, attrId):
    print(f'attr Deleted : {nodeName} at old index : {attrId}')


@QtCore.Slot(str, int, int)
def on_attrEdited(nodeName, oldId, newId):
    print(f'attr Edited : {nodeName} at old index : {oldId}, new index : {newId}')


# Connections
@QtCore.Slot(str, str, str, str)
def on_connected(srcNodeName, srcPlugName, destNodeName, dstSocketName):
    print(f'connected src: "{srcNodeName}" at "{srcPlugName}" to dst: "{destNodeName}" at "{dstSocketName}"')


@QtCore.Slot(str, str, str, str)
def on_disconnected(srcNodeName, srcPlugName, destNodeName, dstSocketName):
    print(f'disconnected src: "{srcNodeName}" at "{srcPlugName}" from dst: "{destNodeName}" at "{dstSocketName}"')
...
if Path(file_path).exists():
    nodz.clearGraph()
    nodz.loadGraph(filePath=file_path)

if __name__ == '__main__':
    # command line stand alone test... run our own event loop
    app.exec_()
    nodz.saveGraph(filePath=file_path)
